...
<?php if ($user_faci[0]['facility_classes_id'] == 2 && $req_info[0]['ju_flg'] == 0): ?>
	<button type="submit" class="btn btn-success" name="order" id="order">依頼を受ける</button><br>
...
<?php if ($user_faci[0]['facility_classes_id'] == 2 && $req_info[0]['ju_flg'] == 1): ?>
	<button type="button" class="btn btn-primary" onclick="location.href='/silver/request/list'">依頼一覧へ</button><br>
