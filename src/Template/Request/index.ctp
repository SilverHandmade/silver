...
<form  action="/silver/request" method="post">
	<p>製作物タイトル
	<input  type="text" id="reqT" name="requestT" value="<?php if ($this->request->is('post')){ echo $_POST["requestT"]; }?>"  maxlength="40" required></p>
	<p>製作個数
	<input type="number" id="reqN" name="requestN" value="1" min="1" max="999" required></p>
	<p>ワークショップID
	<input type="text" id="wsID" name="wsID" value="<?php if ($this->request->is('post')){ echo $_POST["wsID"]; }?>"></p>
	<p>締切日
